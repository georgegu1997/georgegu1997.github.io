...
* Run `bundle install` to install ruby dependencies. If you get errors, delete Gemfile.lock and try again. If you see errors, try `bundle config set --local path vendor/bundle`. 
* Run `bundle exec jekyll serve --livereload --port 4000` to generate the HTML and serve it from `localhost:4000` the local server will automatically rebuild and refresh the pages on change.
